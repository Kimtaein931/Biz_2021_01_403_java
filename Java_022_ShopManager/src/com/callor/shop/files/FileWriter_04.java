...
public class FileWriter_04 {
	/*
	 * 클래스 영역에 변수를 선언하는 것은 현재 클래스가 포함하고 있는 모든 method들이 
	 ...
	 * 
	 * static 변수는 GC 가 접근하지 못한다. 프로젝트가 시작될 때 만들어지고, 계속 존재하는 상태다. 
	 * 프로젝트가 시작될 때 클래스가 자동으로 만들어지고 이때 static 변수들도 자동으로 생성된다. 
	 * 클래스가 생성될 때 같이 만들어진다는 의미로 static 키워드가 부착된
	 * 변수를 '클래스 변수'라고 한다.
	 * 
	 ...
	 * main() method의 호출을 받는
	 * method들은 모두 static으로 선언되어야 한다.
	 * 
	 * main() method와 static으로 선언된 method 들이 클래스 영역에 선언된 변수를 공유하려면 
	 * 클래스 영역에 선언된 변수들도
	...
	private static void saveFileNums() {
		...
		try {
			...
			// intList에 담긴 전체리스트를 향상된 for() 로 반복하면서 파일에 출력
			for (Integer n : intList) {
				printer.println(n);
			}

			// 표준형 for() 로 반복하면서 파일에 출력
