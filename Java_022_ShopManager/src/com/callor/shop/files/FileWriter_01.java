...
public class FileWriter_01 {
	...
	public static void main(String[] args) {
		 ...
		 *  FileWriter 클래스와 PintWriter 클래스를 사용한다.
		 *  클래스를 사용한 객체 선언과 객체 생성 부분을 분리해야 한다.
		 *  왜냐하면 객체를 생성할 때, 사용할 때 try_catch가 의무 적용되어야 하기 때문에
		 *  코드 작성 평의상 분리하는 것이 좋다.
		 *  
		 *  변수를 지금 바로 생성하지 않을 경우
		 *  clear를 해준다.
		 *  숫자형일 경우는 0으로 
		 *  	int num = 0;
		 *  클래스일 경우 null 값으로
		 ...
		 * Java 코딩에서는 이러한 경우 강제의무적으로 예외(상황)처리(try_catch 처리)를 하도록 한다.
		 */
		try {
			 ...
			 * 하지만 저수준 방식은 새로운 방식을 익혀야 할 경우도 있기 때문에 다소 부담스럽다.
			 * fileWriter와 PrintWriter를 연결하면
			 * 평소에 많이 사용하던 System.out.print*() method와 비슷한 방식으로 코드를 작성할 수 있다.
			 * 이러한 방식을 고수준(High level) 방식이라고 한다.
			 * 저수준보다 다소 느리게 작동될 수 있지만
			 * 코드 작성의 부담이 훨씬 줄어든다.
			...
			/*
			 * FileWriter, PrintWriter 등을 사용하여 파일에 내용을 기록한 후에는
			 * 반드시 객체들을 close() 해주어야 한다.
			 * close()를 수행하기 전까진 기록된 데이터들을 운영체제가 임시 보관하고 있다.
			 * 운영체제는 close() 명령이 수행되는 신호를 받으면 비로소 데이터를 파일에 기록한다.
			 */
			printer.close();
			fileWriter.close();
