...
//			Integer num = rnd.nextInt(100) + 1;
	 ...
	 * 
	 * 원칙은 한 클래스 파일 내에서는
	 * 같은 이름의 method를 두 번 이상 정의할 수 없다.
	 * 
	 * 하지만 객체지향 언어에서는
	 * method의 매개변수가 있거나 없거나
	 * 또는 매개변수의 개수가 다르거나
	 * 매개변수의 type이 다르면 같은 이름의 method를
	 * 중복정의(중복하여 정의)할 수 있다.
	 * 
	...
	/*
	 * RndServiceImplV1 클래스를 처음 디자인 할 때는
	 * 파일의 이름을 코드에 직접 작성하였다.
	 * 이후 필요에 의해 파일의 이름을 method 매개변수로 받아서 사용할 수 있도록 변경했다.
	 * 그랬더니 처음 V1 클래스를 호출하여 만들었던 코드들에서 오류가 나타나버렸다.
	 * 
	 * 이 오류를 객체지향의 다형성 원리를 이용하여 해결하였다.
	 * 
	 * 만약 saveFileRnd() 라고 method를 호출하면 코드에 포함된 파일 이름을 적용하여
	 ...
	 * Random 수들이 저장되어있는 파일에서
	 * 
	 * Random 수들을 다시 읽어들여 연산을 수행하는 method
	 * 
	 * 저장할 때 사용한 파일이름 
	 * 		== 읽을 때 사용할 파일이름
	 * 
	 ...
	 * fileName 변수는 인스턴스 영역에 선언되어야 코드가 간편해질 것이다.
	 */
