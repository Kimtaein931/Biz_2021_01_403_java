...
public class Values {
	 ...
	 * 클래스를 사용하여 객체를 선언, 생성하지 않고 변수에 직접 접근할 수 있다.
	 * 
	 ...
	 * 각 항목의 Item 값에 따라 여러 기능을 선택해야하는 코드가 여기저기 나타난다면
	 * 코드를 작성하면서 Item 값(숫자)을 일일이 기억해야만한다.
	 * 이러한 상황은 Item 값을 잘못 지정하여 논리적 오류가 발생할 소지가 많다.
	 * 
	 * 이때 Item 값들을 static 변수로 미리 만들어두고
	 * 필요한 곳에 적용을 하면 코드를 작성하는 도중에 문법적인 검사가 이루어져 논리적 오류를 피할 수 있다.
	 */
