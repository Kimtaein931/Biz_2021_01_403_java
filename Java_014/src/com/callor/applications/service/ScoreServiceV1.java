...
public class ScoreServiceV1 {
	...
	public void init() {
		// ㄴ배열, 객체 등을 초기화하여 사용할 준비를 하는 method
		// Scanner, Random을 선언하는 것과 분리하여 '생성'하는 코드를 작성
		scan = new Scanner(System.in);
		

		System.out.println("몇 명의 학생 성적을 처리할 것인가?");
		System.out.print("학생 수 : ");
		String strNums = scan.nextLine();
		int intNums = Integer.valueOf(strNums);

		// 입력받은 학생 수 만큼 성적 입력 배열 생성
	...
	public void inputScore() {
		...
		for (int i = 0; i < strName.length; i++) {
			...
			intEng[i] = Integer.valueOf(strKor);

			System.out.print("수학 : ");
			strKor = scan.nextLine();
			intMath[i] = Integer.valueOf(strKor);
		}
	...
	public void makeNamesAndArrayInit() {
		...
		strName = new String[] { "홍길동", "이몽룡", "성춘향", "장록수", "임꺽정", "태진아", "설운도", "장윤정" };
		...
		// rnd.nextInt() method를 '호출하기 전에' 반드시 어디에선가 실행되어야 한다.
		rnd = new Random();
	}
