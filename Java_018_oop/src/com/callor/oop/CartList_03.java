...
public class CartList_03 {
	...
	public static void main(String[] args) {
		 ...
		 * 그 곳에 num라는 별명을 부착한다.
		 * 언제나 num = 40 등의 코드를 통해
		 * 변수 값을 변경하고 System.out.println(num) 등을 이용해서
		 * 변수에 저장된 값을 읽을 수 있다.
		 * 
		 * VO 클래스를 사용하여 객체를 선언 및 생성하면
		 * VO 클래스에는 다양한 type의 인스턴스 변수들이 
		 * 포함되어 있기 때문에 변수처럼 한 곳에 모아둘 수가 없다.
		 * 
		...
		// 실제로는 데이터가 아니라 데이터가 저장된 주소값이 추가되는 것
		cartList.add(cartVO);
		...
		for(int i = 0; i < 5; i++) {
			String userName = cartList.get(i).getCartUserName();
			System.out.println("구매자 : " + userName);
		}
