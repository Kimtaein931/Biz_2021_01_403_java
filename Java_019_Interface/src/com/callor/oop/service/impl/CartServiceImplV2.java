...
public class CartServiceImplV2 implements CartService{
	...
	/*
	 *  cartList와 scan 인스턴스 객체변수를 선언'만'하고
	 *  생성은 하지 않았다.
	 * 
	 *  이렇게 하면 생성되지 않은 인스턴스 객체변수의 method를 호출하는 코드에서
	 *  NullPointerException이 발생할 것이다.
	 * 
	 *  NullPointerException을 방지하기 위하여
	 *  클래스 생성자 method에 객체변수를 초기화하는
	 *  코드를 추가해준다.
	 *  
	 *  즉
	 *  인스턴스 객체변수 선언 -> 생성자에서 객체변수 초기화(생성)
	 *  
	 *  이렇게 복잡한 과정으로 코드를 작성하는 이유는
	 *  Java GC (Garbage Collection : 메모리 자동관리)가 작동되는 과정에서 혹시 모를 메모리 누수(leak)를
	 *  방지하기 위한 조치
	 *  
	 *  Java GC : 프로젝트가 실행되는 과정에서 사용이 끝났음에도 아직 메모리에 남아있는 
	 *  			변수, 객체 등을 자동으로 제거하여 메모리 누수를 없애고 관리하는 Java의 기능
	 */
