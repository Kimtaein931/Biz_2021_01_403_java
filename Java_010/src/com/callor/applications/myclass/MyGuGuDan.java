 ...
 * 방지하고 코드를 읽기 쉽고 수정하기 쉽도록 분산하는 방식
 * 
 * main() method에 200줄 정도의 코드가 있다고 할 때
 * 일정한 분량만큼 잘라서 다른 클래스의.메서드()에 코드를 옮기고
 * main()에서는 클래스를 객체로 생성한 다음 객체.메서드() 방식으로
 * 코드를 실행하여 같은 효과를 내는 것
 * 
 */
...
public class MyGuGuDan {
	...
	// print() method는 변수선언명령문을 포함하고있다.
	// 누군가가 MyGuGuDan.print()를 호출(실행)하려면
	// 반드시 정수값을 하나 포함시켜줘야한다.
	// MyGuGuDan.print(정수) 이런 식으로 호출해야한다.
